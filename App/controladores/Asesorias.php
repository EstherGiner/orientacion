...
class Asesorias extends Controlador{
    ...
    public function __construct(){
        ...
        $this->datos['rolesPermitidos'] = [100,200,300];         // Definimos los roles que tendrán acceso
    ...
    public function add_asesoria($error=0){
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!$_POST["nombre_as"] && !$_POST["dni_as"] && !$_POST["titulo_as"] && !$_POST["telefono_as"] && !$_POST["email_as"] && !$_POST["domicilio_as"] && !$_POST["descripcion_as"]){        // Comprobamos que haya datos por lo menos en un campo
                ...
                email_informativo_asesoria($asesoria['email_as'],$asesoria['nombre_as']);   // Envía email al cliente confirmando que ha sido atendido
                
                $asesores = $this->asesoriaModelo->getAsesores();   // Obtenemos todos los asesores disponibles

                $emails = array_column($asesores, 'email');
                $nombres = array_column($asesores, 'nombre_completo');

                email_aviso_asesores($emails,$nombres);     // Enviamos aviso a los asesores para que revisen las nuevas asesorías creadas
    ...
    public function get_accion($id_reg_acciones){
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'GET') {         // Solo acceso GET
            $datos = $this->asesoriaModelo->getAccion($id_reg_acciones);    // No necesitamos la información que nos aporta $this->datos
            $this->vistaApi($datos);
        }
    }
    ...
    public function set_accion(){
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {         // Sólo acceso POST
            $accion = $_POST;
    ...
    public function filtro(){
        if ($_SERVER['REQUEST_METHOD'] == 'GET') {
            ...
            $this->vista("asesorias/filtro",$this->datos);
        } else {
    ...
    public function generar_informe(){
       ...
       $asesorias= $this->asesoriaModelo->getAsesorias();
        generar_informe_pdf($asesorias);
    }
